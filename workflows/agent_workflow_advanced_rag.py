...
def _build_trust_explanation(
    ...
    if not integrity_passed:
        parts.append(
            "CRITICAL: The self-healing loop deleted one or more requested "
            "resources to artificially pass validation. This violates resource integrity."
        )
...
def trust_assessor_node(state: AgentState):
    ...
    explanation = _build_trust_explanation(
        retrieval_sim, reranker_norm, is_valid, retry_count, score, integrity_passed
    )
